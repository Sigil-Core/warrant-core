...
function validateConsumerCompatibilityFixture(fixture: unknown): ConsumerCompatibilityCase[] {
  ...
  return fixture.cases.map((candidate, index) => {
    ...
    if (record.versions !== undefined) {
      ...
      versions = record.versions as string[];
      ...
      if (!versions.includes(versionHeader[1]!)) {
    ...
    if (record.outcome === "accept") {
      ...
      return {
        ...base,
        outcome: "accept",
        ...("canonicalPolicy" in record ? { canonicalPolicy: record.canonicalPolicy } : {}),
      };
    }
...
export function assertKnownParityDivergencesCovered(): void {
...
function markdownForVersion(compatibilityCase: ConsumerCompatibilityCase, version: string): string {
  let replaced = false;
...
export function defineConsumerCompatibilityVectorTests(runtime: string): void {
  for (const compatibilityCase of consumerCompatibilityCases) {
    it(`matches the ${compatibilityCase.id} consumer-compatibility vector in ${runtime}`, () => {
      const markdownCases = compatibilityCase.versions?.map((version) =>
        markdownForVersion(compatibilityCase, version),
      ) ?? [compatibilityCase.markdown];
      if (compatibilityCase.outcome === "accept") {
        for (const markdown of markdownCases) {
          const parsed = parsePolicyMarkdown(markdown);
          if ("canonicalPolicy" in compatibilityCase) {
            expect(parsed).toEqual(compatibilityCase.canonicalPolicy);
          }
        }
        return;
      }
