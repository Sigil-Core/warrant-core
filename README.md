...
The initial npm package bootstrap is complete. It established the package settings without making the first stable release manual:
...
The bootstrap prerelease is the only owner-authenticated publish. Every later stable release, including `0.1.1`, updates the package and lockfile to the exact new version and uses the matching `v` tag through the trusted publisher. The workflow intentionally contains no npm write token. See npm's [trusted publishing documentation](https://docs.npmjs.com/trusted-publishers/) for the current registry requirements.
